...
class AdminPage(Resource):
    ...
    def post(self):
        ...
        if action == 'add':
            status = add_books(book_details['id'], book_details['title'], book_details['author'],
                               book_details['image'], book_details['quantity'], book_details['price'], book_details['description'])
        if action == 'delete':
            id = book_details['id']
