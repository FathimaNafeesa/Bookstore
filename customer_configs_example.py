"""
Example Customer Configurations
Each customer passes their own data as inputs to the generic script
"""

# =============================================================================
# KNF CUSTOMER CONFIG
# =============================================================================

knf_inputs = {
    # Basic inputs
    "rawLog": "{}",  # JSON string from playbook
    "udmEvent": "{}",  # JSON string from playbook
    "alertName": "some_alert",
    
    # Simple whitelist lists
    "whitelist_alert_names": [
        "This alert is triggered when any email message is reported as junk by users -V1.0.0.0",
        "Malicious emails were delivered and later removed -V1.0.0.2",
        "Emails with malicious URL that were delivered and later removed -V1.0.0.3",
        "Email messages containing malicious URL removed after delivery",
        "Email messages containing malicious file removed after delivery",
        "Email reported by user as junk",
        "Email reported by user as not junk",
        "Email messages from a campaign removed after delivery",
        "Email messages from a campaign were delivered and later removed -V1.0.0.2"
    ],
    
    "whitelist_hashes": [
        "FE7A9EA0905151A907E248E91655EC36D9481BF7DB839ADED6D0444A66D8D730"
    ],
    
    "whitelist_ips": [
        "83.135.49.100", "5.45.7.100", "178.174.74.158", "93.64.21.162",
        "109.164.253.18", "62.84.220.189", "182.74.194.122", "81.62.137.122",
        "112.217.96.26", "195.67.15.34", "83.144.238.14", "81.62.221.10",
        "118.238.219.96", "124.110.102.16", "12.26.44.228", "108.58.132.154",
        "77.159.255.36", "90.80.222.57", "193.117.132.34", "1.119.186.210",
        "101.204.45.250", "210.13.71.132", "116.227.23.83", "219.134.241.42"
    ],
    
    "whitelist_categories": ["SuspiciousActivity"],
    
    "whitelist_udm_event_types": [
        "MCAS_ALERT_CABINET_EVENT_MATCH_FILE",
        "MCAS_ALERT_CABINET_EVENT_MATCH_AUDIT"
    ],
    
    "whitelist_descriptions": [
        "This alert is triggered when any email message is reported as junk by users -V1.0.0.0",
        "Malicious emails were delivered and later removed -V1.0.0.2",
        "Email messages containing malicious URL removed after delivery",
        "Email messages containing malicious file removed after delivery",
        "Email reported by user as junk",
        "Email reported by user as not junk"
    ],
    
    # API keys
    "vt_api_key": "your_virustotal_api_key",
    
    # Complex rules
    "custom_rules": [
        {
            "type": "category_ip_description",
            "params": {
                "category": "SuspiciousActivity",
                "ips": [
                    "83.135.49.100", "5.45.7.100", "178.174.74.158", "93.64.21.162",
                    "109.164.253.18", "62.84.220.189", "182.74.194.122", "81.62.137.122",
                    "112.217.96.26", "195.67.15.34", "83.144.238.14", "81.62.221.10",
                    "118.238.219.96", "124.110.102.16", "12.26.44.228", "108.58.132.154",
                    "77.159.255.36", "90.80.222.57", "193.117.132.34", "1.119.186.210",
                    "101.204.45.250", "210.13.71.132", "116.227.23.83", "219.134.241.42"
                ],
                "description_contains": "Mass download by a single user"
            }
        },
        {
            "type": "alert_description_match",
            "params": {
                "alert_name": "InitialAccess",
                "descriptions": [
                    "This alert is triggered when any email message is reported as junk by users -V1.0.0.0",
                    "Malicious emails were delivered and later removed -V1.0.0.2",
                    "Emails with malicious URL that were delivered and later removed -V1.0.0.3"
                ]
            }
        },
        {
            "type": "virustotal_domain",
            "params": {
                "alert_name": "External_user_added_to_Teams_M365",
                "domain_path": "target.administrativeDomain",
                "domain_age_days": 90,
                "malicious_threshold": 10
            }
        }
    ]
}


# =============================================================================
# HENGST CUSTOMER CONFIG
# =============================================================================

hengst_inputs = {
    # Basic inputs
    "rawlog": "{}",  # Note: lowercase
    "alertName": "NTT_azure_multiple_failed_logins",
    
    # Simple whitelist lists (empty for this customer)
    "whitelist_alert_names": [],
    "whitelist_hashes": [],
    "whitelist_ips": [],
    
    # API keys
    "abuse_key": "your_abuseipdb_api_key",
    
    # Complex rules
    "custom_rules": [
        {
            "type": "abuseipdb_isp",
            "params": {
                "alert_name": "NTT_azure_multiple_failed_logins",
                "isp_contains": "hengst"
            }
        }
    ]
}


# =============================================================================
# XELLA CUSTOMER CONFIG
# =============================================================================

xella_inputs = {
    # Basic inputs
    "rawLog": "{}",
    "alertName": "some_alert",
    
    # No whitelisting rules yet for this customer
    "whitelist_alert_names": [],
    "whitelist_hashes": [],
    "whitelist_ips": [],
    "custom_rules": []
}


# =============================================================================
# HOW TO USE IN PLAYBOOK
# =============================================================================

"""
In your playbook, you would pass these inputs like:

For KNF:
    sw_context.inputs = {
        "rawLog": actual_raw_log,
        "udmEvent": actual_udm_event,
        "alertName": actual_alert_name,
        "whitelist_alert_names": knf_inputs["whitelist_alert_names"],
        "whitelist_hashes": knf_inputs["whitelist_hashes"],
        "whitelist_ips": knf_inputs["whitelist_ips"],
        "custom_rules": knf_inputs["custom_rules"],
        "vt_api_key": knf_inputs["vt_api_key"],
    }

For HENGST:
    sw_context.inputs = {
        "rawlog": actual_raw_log,
        "alertName": actual_alert_name,
        "abuse_key": hengst_inputs["abuse_key"],
        "custom_rules": hengst_inputs["custom_rules"],
    }

Then run generic_whitelist.py
"""
